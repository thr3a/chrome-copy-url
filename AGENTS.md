Manifest V3のChrome拡張機能テンプレート。esbuildでビルドし、成果物は `dist/` に出力される。
FirefoxやEdgeは考慮せずChromeのみだがwebextension-polyfill使用している。
...
- `manifest.json` の `permissions` は最小限に保つ
- 【超重要】 `webextension-polyfill` 経由で使用（`import browser from 'webextension-polyfill'`）。Chrome固有APIは直接使わない。
  - 間違い: chrome.runtime.onMessage.addListener
  - 正解: browser.runtime.onMessage.addListener
